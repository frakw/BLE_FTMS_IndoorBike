...
fire_and_forget SubscribeCharacteristicAsync_Read(wchar_t* deviceId, wchar_t* serviceId, wchar_t* characteristicId, bool* result) {
	try {
...
fire_and_forget SubscribeCharacteristicAsync_Write(wchar_t* deviceId, wchar_t* serviceId, wchar_t* characteristicId, bool* result) {
	try {
		auto characteristic = co_await retrieveCharacteristic(deviceId, serviceId, characteristicId);
		if (characteristic != nullptr) {
...
bool SubscribeCharacteristic_Read(wchar_t* deviceId, wchar_t* serviceId, wchar_t* characteristicId, bool block) {
	unique_lock<mutex> lock(subscribeQueueLock);
	bool result = false;
	SubscribeCharacteristicAsync_Read(deviceId, serviceId, characteristicId, block ? &result : 0);
...
bool SubscribeCharacteristic_Write(wchar_t* deviceId, wchar_t* serviceId, wchar_t* characteristicId, bool block) {
	unique_lock<mutex> lock(subscribeQueueLock);
	bool result = false;
	SubscribeCharacteristicAsync_Write(deviceId, serviceId, characteristicId, block ? &result : 0);
	if (block && QuittableWait(subscribeQueueSignal, lock))
		return false;
